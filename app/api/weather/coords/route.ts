...
export async function GET(request: NextRequest) {
  ...
  try {
    const [currentResponse, forecastResponse] = await Promise.all([
      fetch(`${BASE_URL}/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=imperial`),
      fetch(`${BASE_URL}/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=imperial`)
    ])
    ...
    return NextResponse.json({
      current: normalizeCurrentWeather(currentData),
      forecast: normalizeForecastList(forecastData.list, forecastData.city?.timezone ?? 0)
    })
  } catch (error) {
